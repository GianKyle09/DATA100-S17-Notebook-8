...
class KMeans(object):
    ...
    def get_euclidean_distance(self, point1, point2):
        ...
        if isinstance(point1, pd.Series) and isinstance(point2, pd.Series):
            return np.sqrt(((point1 - point2) ** 2).sum().astype(np.float64))
        elif isinstance(point1, pd.DataFrame) and isinstance(point2, pd.Series):
            return np.sqrt(((point1 - point2) ** 2).sum(axis=1).astype(np.float64))
        else:
            raise TypeError("Invalid combination of data types for point1 and point2")
    ...
    def adjust_centroids(self, data, groups):
        ...
        numeric_columns = grouped_data.columns.difference(['group'])
